...
int main(int argc , char ** argv){
    ...
    while (!SelfConsistency){
        ...
        SelfConsistency = DFTDensityUpdate(N, density, refdensity, evec, threshold);
        if (SelfConsistency){
            std::cout << "DFT Self consistency reached in " << counter << " iterations." << std::endl << std::endl;
        }
        if (counter == max_it){
            std::cout << "DFT Self consistency not reached. Iteration = " << counter << std::endl << std::endl;
            break;
        }
        counter += 1;
    }
    ...
    std::cout << "Calculation took : " << duration.count() << " microseconds." << std::endl;
